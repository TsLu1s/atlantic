...
class DatasetGenerator:
        ...
        for col in df.columns:
            ...
            if n_nulls > 0:
                null_indices = np.random.choice(
                    df.index, size=n_nulls, replace=False
                )
    ...
    def generate_classification(
        ...
        categories = DatasetGenerator.CATEGORY_POOLS.get(
            ...
            DatasetGenerator.CATEGORY_POOLS['low']
        )
    ...
    def generate_high_cardinality(
        ...
        for i in range(n_categorical):
            cardinality = np.random.randint(
                cardinality_range[0], 
                cardinality_range[1]
            )
            categories = [f'hc_cat_{i}_{j}' for j in range(cardinality)]
            df[f'high_card_feature_{i}'] = np.random.choice(
                categories, size=n_samples
            )
