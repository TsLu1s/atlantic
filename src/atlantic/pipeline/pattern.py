...
class Pattern(TargetTypeMixin):
    ...
    def encoding_selection(self) -> str:
        ...
        """
        result = self._encoding_optimizer.select_best_version(
            self.train.copy(),
            self.test.copy()
        )
    ...
    def imputation_selection(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        if self._verbosity >= 1:
            ...
            print(f"\n{best_method} Imputation selected with "
                  f"{eval_metric} of: {self.perf:.4f}")
    ...
    def vif_performance(
        ...
        vif_selector = VIFFeatureSelector(
            target=self.target,
