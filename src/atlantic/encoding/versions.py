...
class EncodingVersionFactory:
    ...
    def get_version_config(cls, version: str) -> Tuple[str, str]:
        ...
        if version_lower not in cls.VERSIONS:
            raise ValueError(
                ...
                f"Available: {list(cls.VERSIONS.keys())}"
            )
    ...
    def apply_version(
        ...
        # Get column types
        num_cols = [
            col for col in train.select_dtypes(
    ...
    def describe_version(cls, version: str) -> str:
        ...
        """
        version_lower = version.lower()
        return cls.VERSION_DESCRIPTIONS.get(
            version_lower, 
            f"Unknown version: {version}"
        )
